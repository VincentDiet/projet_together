...
const renderStars = computed(() => {
  const notes = [];
  props.activity.author.activities.forEach((activity) => {
    activity.participations.forEach((participation) => {
      if (participation.organizator_note) {
        notes.push(participation.organizator_note);
      }
    });
  });
  let average = notes.reduce((a, b) => a + b, 0) / notes.length || 0;

  const solidStar = `<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-yellow-500" viewBox="0 0 24 24"><path fill="currentColor" d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2L9.19 8.63L2 9.24l5.46 4.73L5.82 21L12 17.27z"/></svg>`;
  const outlineStar = `<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-yellow-400" viewBox="0 0 24 24"><path fill="currentColor" d="m22 9.24l-7.19-.62L12 2L9.19 8.63L2 9.24l5.46 4.73L5.82 21L12 17.27L18.18 21l-1.63-7.03L22 9.24zM12 15.4l-3.76 2.27l1-4.28l-3.32-2.88l4.38-.38L12 6.1l1.71 4.04l4.38.38l-3.32 2.88l1 4.28L12 15.4z"/></svg>`;
  let stars = ``;
  for (let i = 1; i <= 5; i++) {
    if (i <= Math.round(average)) {
      stars += solidStar; // Ajouter une étoile pleine
    } else {
      stars += outlineStar; // Ajouter une étoile vide
    }
  }
  if (notes.length === 0) {
    stars = `<span class="text-yellow-100">Non noté</span>`;
  }
  return stars;
});
</script>
<template>
  <li class="flex-shrink-0 mr-3 w-56 max-w-sm rounded-xl overflow-hidden shadow-lg mb-4">
    ...
    <div class="relative">
      ...
      <div class="absolute top-3 left-3 bg-white py-1 px-2 rounded-lg">
        <span class="text-together-cyan-dark font-extrabold">{{ activity.category.name }}</span>
      </div>
      <!-- Icône -->
      <div class="absolute top-3 right-3 bg-white rounded-full p-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-900" viewBox="0 0 24 24">
          <path fill="currentColor" d="M5 21V5q0-.825.588-1.413T7 3h10q.825 0 1.413.588T19 5v16l-7-3l-7 3Zm2-3.05l5-2.15l5 2.15V5H7v12.95ZM7 5h10H7Z" />
        </svg>
      </div>
      <!-- Date -->
    ...
    <div class="bg-together-blue py-4 px-3">
      <!-- Titre -->
      <h2 class="text-white text-lg font-bold mb-2 truncate">{{ activity.title }}</h2>
      <!-- Icônes avec texte -->
      <ul>
        <li class="flex items-center flex-row text-together-green font-bold mb-1">
        ...
        <li class="flex items-center flex-row font-bold mb-1">
          ...
          <div class="flex ml-2 items-center" v-html="renderStars"></div>
        </li>
        <li class="flex items-center flex-row font-bold mb-1">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 24 24">
            <path
      ...
      <div class="flex justify-center items-center">
        <a :href="route('activities.show', { id: activity.id })" class="bg-together-green text-white font-bold rounded-full py-1.5 mt-3 px-8"> Infos </a>
      </div>
    </div>
  </li>
</template>
