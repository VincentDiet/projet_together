  ...
  <AppLayout title="Add an activity" class="bg-gray-100">
    ...
    <form class="flex flex-col gap-4 text-gray-700 pt-8">
      <div class="h-96 overflow-hidden">
        <img :src="formData.image" alt="" class="object-cover h-96" :class="!formData.image ? 'hidden' : ''" />
        <img src="https://www.likeminds.fr/wp-content/uploads/2015/12/Point-dinterrogation-Gris-1.png" alt="" class="object-cover h-96" />
      </div>
      <Input @input="onInput" label="Image URL" name="photo" placeholder="url"></Input>
          ...
          <template v-for="category in categories" :key="category.id">
            ...
            <label :for="category.id">
              <div
                ...
                :class="formData.categorie_id === category.id ? 'bg-green-500 text-white' : ''"
      ...
      <Input @input="onInput" label="Date de l'activité" name="date" placeholder="01/01/2023"></Input>
      <Input @input="onInput" label="Heure de l'activité" name="time" placeholder="10:00"></Input>
      <Input @input="onInput" label="Durée de l'activité" name="duration" placeholder="1"></Input>
      <Input @input="onInput" label="Nombre de participants" name="max_participants" placeholder="5"></Input>
      <Input @input="onInput" label="Adresse" name="adress" placeholder="Adresse de l'activité"></Input>
      <Input @input="onInput" label="Ville" name="city" placeholder="Ville"></Input>
      <Input @input="onInput" label="Pays" name="country" placeholder="Pays"></Input>
      <label class="block font-medium text-sm text-gray-700" :for="name">
        <p class="font-bold">{{ label }}</p>
        <p class="font-bold">Description</p>
        <div class="flex flex-row justify-between rounded-xl border-0 py-1 px-2 w-full bg-together-green-light">
          <textarea
            ...
            rows="7"
          />
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mx-1" viewBox="0 0 24 24">
            <path
              fill="currentColor"
      ...
      <input type="button" value="Envoyer" class="bg-green-500 py-2 font-bold text-white rounded-full my-4 disabled:bg-gray-300 disabled:text-gray-200" @click="submitForm" />
    </form>
  </AppLayout>
</template>
